...
def make_config(path=None):
    ...
    config['fooof_freq_range_how'] = 40
    # which mode should be used for fitting; choices (knee, fixed)
    config["aperiodic_mode"] = "fixed"
...
def merge_fidp_demo(
    ...
    if not os.path.exists(feature_path):
        raise FileNotFoundError(f"The file 'all_features.csv' is missing in the directory: {features_dir}.")
    ...
    for col in columns:
        if col in df.columns:
            if col == "diagnosis":
                ...
                df["diagnosis"] = np.where(df["diagnosis"] == "control", 0, pd.factorize(df["diagnosis"])[0] + 1)
            else:
...
def normalize_column(df, column="age", normalizer=100):
    ...
    if normalizer is not None and (not isinstance(normalizer, (int, float)) or normalizer <= 0):
        raise ValueError(f"Normalizer should be a positive numeric value, got {normalizer}.")
    ...
    *extra_columns : dict
    ...
    for col in columns:    
        if col.get("single_value") and col.get("mapping"):
            raise ValueError("'single_value' and 'mapping' can not be both defined. One of them must be None; see the documentation!")
