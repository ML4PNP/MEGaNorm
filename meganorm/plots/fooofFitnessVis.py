...
def plot_knee_fixed_compare_psd(freqs, psds, fixed_aperiodics, knee_aperiodics, save_path):
    """
...
def compute_fittness_metric(paths):
    ...
    for counter, path in enumerate(paths):
        ...
        for i  in range(fooofres[1].shape[0]):
            fm = fooofres[0].get_fooof(ind=i)
            errors.append(fm.error_); r2s.append(fm.r_squared_); n_peaks.append(fm.n_peaks_)
...
def plot_ap_fixed_r2(fixed_r2s, knee_r2s, save_path):
    ...
    plt.hist(fixed_r2s, bins=100, color="#A9561E", alpha=0.6, label="Aperiodic - fixed mode")
...
def plot_log_log_fooof(freqs, psds, fixed_ap, knee_ap, save_path):
    ...
    plt.plot(np.log10(freqs), np.log10(fixed_ap), color="#A9561E", label="Aperiodic - fixed mode")
    plt.plot(np.log10(freqs), np.log10(knee_ap), color="teal", label="Aperiodic - Knee mode")
    plt.grid()
    plt.xlabel("Log(Frequency)", fontsize=20); plt.ylabel("log(Power)", fontsize=20)
    plt.yticks(fontsize=18); plt.xticks(fontsize=18)
    plt.gca().spines["right"].set_visible(False)
    plt.gca().spines["top"].set_visible(False)
    plt.legend(fontsize=16)
    plt.savefig(os.path.join(save_path, "loglogfooof.png"), dpi=600, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    # Analyses 1 ----------------------------------------------------------------------
    fixed_path = glob.glob("/home/meganorm-mznasrabadi/Data/CamCAN/fooofResultsNoKnee/sub-CC*")
    ...
    (freqs, psds_avg, fixed_periodics_avg, 
     fixed_aperiodics_avg) = averaged_psds_component(fixed_path)   

    (_, _, knee_periodics_avg, 
     knee_aperiodics_avg) = averaged_psds_component(knee_path)

    # plot aperiodic, periodic and PSDs (NOT in log log space)
    plot_knee_fixed_compare_psd(freqs, psds_avg, fixed_aperiodics_avg, knee_aperiodics_avg, save_path)
    ...
    plot_log_log_fooof(freqs, psds_avg, fixed_aperiodics_avg, knee_aperiodics_avg, save_path)