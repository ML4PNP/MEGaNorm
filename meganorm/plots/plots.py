...
def plot_age_dist2(
    ...
    plt.savefig(
        os.path.join(save_path, "age_dis.svg"),
        format="svg",
        dpi=600,
        bbox_inches="tight",
    )
    plt.savefig(
        os.path.join(save_path, "age_dis.png"),
        format="png",
        dpi=600,
        bbox_inches="tight",
    )
...
def plot_growthchart(age_vector, centiles_matrix, cut=0, idp="", save_path=None):
    ...
    if save_path is not None:
        plt.savefig(
            os.path.join(save_path, idp.replace(" ", "_") + "_growthchart.png"), dpi=600
        )
...
def box_plot_auc(df, save_path):
    ...
    sns.boxplot(
        x="variable", y="value", data=data_long, color="lightgray"
    )  # , palette=colors)
