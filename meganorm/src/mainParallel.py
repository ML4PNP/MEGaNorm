...
def main(args):
    ...
    if configs.save_psds:
        # Two-panel figure: top = raw/preprocessed/rejected/SL PSDs (via
        # computePsdIrasa for IRASA runs, matching its internal settings) +
        # aperiodic fit (normalized to match the stage it was fit on) +
        # knee-frequency marker reused from `feature_extract`'s own
        # aperiodic fit above (not refit here); bottom = periodic component
        # alone, clipped to its own mean +/- 4 SD.
        psd_stage_report(
    ...
    # hardcoded temprorary (just for test) xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    features, _ = feature_extract(
        subject_id=args.subject,
        spectral_models=spectral_models,
        psds=psds,
        freqs=freqs,
    ...
    temp_path = "/project/meganorm/Share/Zaman/Second_paper/psd_parametrization_results/knee_mode_fixed_with_pyrasa"
    os.makedirs(temp_path, exist_ok=True)
    features.to_csv(os.path.join(temp_path, f"{args.subject}.csv"))
    ...
    temp_path = "/project/meganorm/Share/Zaman/Second_paper/psd_parametrization_results/knee_mode_knee_with_pyrasa_hset_2"
    os.makedirs(temp_path, exist_ok=True)
    features.to_csv(os.path.join(temp_path, f"{args.subject}.csv"))
    ...
    temp_path = "/project/meganorm/Share/Zaman/Second_paper/psd_parametrization_results/knee_mode_fixed_with_pyrasa_hset_2"
    os.makedirs(temp_path, exist_ok=True)
    features.to_csv(os.path.join(temp_path, f"{args.subject}.csv"))
